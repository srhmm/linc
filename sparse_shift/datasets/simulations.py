...
def sample_cdnod_sim(
    ...
    iv_hard = False, iv_scale = False, iv_coef = False, iv_fun=False, iv_shift=False,
    noise_uniform=True, base_coef=False
):
    """
    Simulates data from a given dag according to the simulation design
    in Huang et al. 2020
    ...
    domain_equations = [
        partial(
            ...
            parents=parents,
            #coefs=[1]*sum(parents),
            coefs=_coefs_iv(parents), #domain_seed.uniform(0.5, 2.5, size=(np.sum(parents))),
