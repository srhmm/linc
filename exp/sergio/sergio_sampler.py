...
from exp.sergio.sergio_mod import Sergio as SergioSimulator
...
def grn_sergio(
    *,
    ...
    if effect_sgn is None:
        effect_sgn = (
            rng.binomial(1, k_sign_p(rng, shape=(n_vars, 1)), size=g.shape) * 2.0 - 1.0
        )
    else:
    ...
    if simulate_interv_data:
        assert (
            n_ko_genes != 0
        ), f"Need n_ko_genes != 0 to have interventional data for SERGIO"
        if n_ko_genes == -1:
            n_ko_genes = n_vars
        ko_targets += sorted(
            rng.choice(n_vars, size=min(n_vars, n_ko_genes), replace=False).tolist()
        )
    ...
    for e, ko_target in enumerate(ko_targets):
        ...
        if ko_target is None:
            ...
            number_sc = math.ceil(
                spec["n_observations_int"] / (n_cell_types * n_ko_genes)
            )
        ...
        assert (
            number_sc >= 1
        ), f"Need to have number_sc >= 1: number_sc {number_sc} data_type {data_type}"
        sim.simulate()
    ...
    assert x_obs.size != 0 or x_int.size != 0, (
        f"Need to sample at least some observations; "
